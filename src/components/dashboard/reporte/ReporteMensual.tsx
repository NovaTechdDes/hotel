        ...
        <div>
            ...
            <div className='grid grid-cols-2 gap-10 mt-5'>
                <div className='flex flex-col gap-2'>
                    <label htmlFor="mes0" className='text-lg font-semibold'>Seleccionar Mes</label>
                    <select className='border rounded-lg px-2 py-1 text-lg border-gray-300 w-full' onChange={(e) => setMes(e.target.value)} name="mes" value={mes} id="mes">
                        {meses.map((elem) => (
                            <option value={elem.value} key={elem.value}>{elem.text}</option>
                        ))}
                    </select>
                </div>
                <div className='flex flex-col gap-2'>
                    <label htmlFor="anio" className='text-lg font-semibold'>Seleccionar Año</label>
            ...
            <div className='md:grid-cols-3 gap-5 grid p-5'>
                ...
                <TarjetaReporte title='Ingreso Promedio' Icon={VscGraph} colorIcon='blue' numero={`$${reservasMes.length !== 0 ? (totalIngresos / reservasMes.length).toFixed(2) : '0.00'}`} text='Por Reserva' />
            </div>
