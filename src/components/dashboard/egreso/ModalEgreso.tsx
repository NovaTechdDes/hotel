            ...
            <div className="bg-white rounded-lg shadow-lg w-xl h-[80vh] p-8 text-black flex flex-col">
                ...
                <form className="grid grid-cols-2 gap-1 flex-1 min-h-0" onSubmit={handleSubmitForm}>
                    ...
                    <div className='col-span-2'>
                        ...
                        <input autoFocus type="text" name="descripcion" value={descripcion} onChange={onInputChange} id="descripcion" className='w-full border rounded-md px-3 py-2' placeholder='Descripcion del egreso...' />
                        {error && descripcion === '' && <p className='text-red-500'>La descripcion es obligatoria</p>}
                    </div>

                    <div>
                        <label htmlFor="creado_en" className='block text-sm font-medium mb-1'>Fecha</label>
                        <input type="date" name="creado_en" value={creado_en?.slice(0, 10) ?? '0000-00-00'} onChange={onInputChange} id="creado_en" className='w-full border rounded-md px-3 py-2' />
                    </div>

                    <div>
                        <label htmlFor="importe" className='block text-sm font-medium mb-1'>Importe *</label>
                    ...
                    <div className='col-span-2'>
                        ...
                        <select name="tipoegresoid" value={tipoegresoid} onChange={onInputChange} id="tipoegresoid" className='w-full border rounded-md px-3 py-2'>
                            ...
                            {tipoEgresos?.map(elem => (
                                <option key={elem.id} value={elem.id}>{elem.descripcion}</option>
                            ))}
                        </select>
                    ...
                    <div className="flex justify-end col-span-2  mt-auto gap-2">
                        ...
                        {!egresoSeleccionado && <button type="submit" disabled={isPendingAgregar} className="px-4 py-2 bg-black text-white rounded hover:opacity-80 cursor-pointer transition" >
                            {isPendingAgregar ? 'Guardando...' : 'Guardar'}
                        </button>}

                        {egresoSeleccionado && <button type="submit" disabled={isPendingModificar} className="px-4 py-2 bg-black text-white rounded hover:opacity-80 cursor-pointer transition" >
