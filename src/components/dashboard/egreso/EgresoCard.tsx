    ...
    return (
        <div className={`bg-white text-black p-5 m-5 rounded-lg shadow-xl ${isPending ? "opacity-70 pointer-events-none relative" : ""}`}>
            ...
            <div className="flex justify-center items-center border-t border-gray-300 mt-5 pt-5 gap-5">
                <button disabled={isPending} className="flex  gap-2 border border-gray-300 rounded-lg items-center px-8 py-1 hover:bg-gray-200 cursor-pointer" onClick={handlePut}>
                    <BiPencil />
                    Editar
                </button>

                {user && user.rol === 'admin' && (
                    <button disabled={isPending} onClick={handleDelete} className="flex  gap-2 border border-gray-300 rounded-lg items-center px-8 py-1 cursor-pointer text-red-500 hover:bg-gray-200">
                        <MdDeleteOutline />
                        Eliminar
                    </button>
                )
